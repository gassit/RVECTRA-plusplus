  ...
  sections: [{
    ...
    children: [
        ...
        rows: [
          ...
          new TableRow({ children: [
            new TableCell({ borders: cellBorders, shading: { fill: colors.tableBg, type: ShadingType.CLEAR }, verticalAlign: VerticalAlign.CENTER,
              children: [new Paragraph({ children: [new TextRun({ text: "Описание", bold: true })] })] }),
            new TableCell({ borders: cellBorders, verticalAlign: VerticalAlign.CENTER,
              children: [new Paragraph({ children: [new TextRun("Сервер разработки Next.js периодически останавливается без видимых причин. Требуется частый перезапуск для продолжения работы. При старте иногда появляется ошибка «Error: $ next dev».")] })] })
          ]}),
          new TableRow({ children: [
            new TableCell({ borders: cellBorders, shading: { fill: colors.tableBg, type: ShadingType.CLEAR }, verticalAlign: VerticalAlign.CENTER,
              children: [new Paragraph({ children: [new TextRun({ text: "Решение", bold: true })] })] }),
        ...
        rows: [
          ...
          new TableRow({ children: [
            new TableCell({ borders: cellBorders, shading: { fill: colors.tableBg, type: ShadingType.CLEAR }, verticalAlign: VerticalAlign.CENTER,
              children: [new Paragraph({ children: [new TextRun({ text: "Описание", bold: true })] })] }),
            new TableCell({ borders: cellBorders, verticalAlign: VerticalAlign.CENTER,
              children: [new Paragraph({ children: [new TextRun("Компонент NetworkGraph отображает «Загрузка данных сети...» бесконечно. API /api/network возвращает корректные данные (194 элемента, 202 соединения), но фронтенд не получает или не обрабатывает их. При проверке API напрямую — данные возвращаются корректно.")] })] })
          ]}),
          new TableRow({ children: [
            new TableCell({ borders: cellBorders, shading: { fill: colors.tableBg, type: ShadingType.CLEAR }, verticalAlign: VerticalAlign.CENTER,
              children: [new Paragraph({ children: [new TextRun({ text: "Диагностика", bold: true })] })] }),
            new TableCell({ borders: cellBorders, verticalAlign: VerticalAlign.CENTER,
              children: [new Paragraph({ children: [new TextRun("Проверено: API работает (GET /api/network 200), данные возвращаются. Страница загружается (GET / 200). Проблема на стороне клиента — требуется проверка консоли браузера на наличие ошибок JavaScript.")] })] })
          ]}),
          new TableRow({ children: [
            new TableCell({ borders: cellBorders, shading: { fill: colors.tableBg, type: ShadingType.CLEAR }, verticalAlign: VerticalAlign.CENTER,
              children: [new Paragraph({ children: [new TextRun({ text: "Решение", bold: true })] })] }),
            new TableCell({ borders: cellBorders, verticalAlign: VerticalAlign.CENTER,
              children: [new Paragraph({ children: [new TextRun("1) Проверить консоль браузера на ошибки; 2) Проверить сетевые запросы в DevTools; 3) Возможна проблема с клиентским рендерингом G6.")] })] })
          ]})
        ]
      }),
      new Paragraph({ spacing: { after: 200 }, children: [] }),
      ...
      new Paragraph({ indent: { firstLine: 480 }, children: [
        ...
        new TextRun("Ошибка #4 является наиболее критичной, так как блокирует основной функционал приложения — визуализацию электросети. Сервер работает, API возвращает данные, но клиентская часть не отображает граф.")
      ]}),
      
      new Paragraph({ spacing: { after: 300 }, children: [] }),
      new Paragraph({ heading: HeadingLevel.HEADING_1, children: [new TextRun("Следующие шаги")] }),
      new Paragraph({ numbering: { reference: "bullet-list", level: 0 }, children: [
        new TextRun("Проверить консоль браузера на наличие ошибок JavaScript при загрузке страницы")
      ]}),
      new Paragraph({ numbering: { reference: "bullet-list", level: 0 }, children: [
        new TextRun("Проверить сетевые запросы в DevTools — убедиться, что API вызывается корректно")
      ]}),
      new Paragraph({ numbering: { reference: "bullet-list", level: 0 }, children: [
        new TextRun("Проверить совместимость версии @antv/g6 с кодом компонента NetworkGraphInner")
      ]}),
      new Paragraph({ numbering: { reference: "bullet-list", level: 0 }, children: [
        new TextRun("Синхронизировать пути к базе данных в .env и schema.prisma")
      ]})
    ]
  }]
});
