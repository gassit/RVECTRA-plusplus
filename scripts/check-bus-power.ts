/**
 * ============================================================================
 * ДИАГНОСТИКА BUS ЭЛЕМЕНТОВ
 * ============================================================================
 *
 * Запуск: npx ts-node scripts/check-bus-power.ts
 *
 * Проверяет:
...
async function main() {
  ...
  console.log('-'.repeat(60));
  console.log(`Всего: ${(buses as any[]).length} (ON: ${onCount}, OFF: ${offCount})`);
  ...
  const problematicBuses = (buses as any[]).filter(
  ...
  if (problematicBuses.length === 0) {
    ...
    for (const bus of problematicBuses) {
      ...
      if ((outgoing as any[]).length === 0) {
        console.log('   ❌ НЕТ СВЯЗЕЙ - нет нагрузок');
      } else {
        for (const c of outgoing as any[]) {
          console.log(`   -> ${c.type} "${c.name?.substring(0, 25)}" P=${c.sumPCalculated || 0} [${c.operationalStatus || 'NULL'}]`);
        }
      }
    }
  }
  ...
  for (const bus of busesWithLoads as any[]) {
    ...
    console.log(`\n📍 ${bus.name?.substring(0, 35)} [${status}]`);
